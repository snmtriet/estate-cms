...
export const Inventory = () => {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = inventoryData.map((n) => n._id);
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - inventoryData.length) : 0;
      ...
      <Paper sx={{ width: "100%", mb: 2 }}>
        ...
        <TableContainer>
          ...
          >
            ...
            />
            <TableBody>
              {stableSort(inventoryData, getComparator(order, orderBy))
